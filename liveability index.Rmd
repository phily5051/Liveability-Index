...
# save nb_entropy
# st_write(nb_entropy, 'C:/Users/phily/liveability-index/data/nb_netropy.gpkg')
```
...
This approache captures the average distance from the neighborhood centre to the urban service-related amenities as a whole.
...
# pull the nb_clus which does not have living facilities - 0 origin 
...
rank_exp <- function(df, var){
  ...
  var <- enquo(var) # https://public-bigdata.tistory.com/48
  # select relevant data from df
  data <- select(df, nb_clus, !! var)
  # create a ranking, rescale to 0-1, then apply exponential transformation
  data %>%
...
# file_path = 'C:/Users/phily/liveability-index/data/uncertainty analysis/ua_rank_exp_450.csv'
# write_csv(ua_output_exp, file_path)
```
...
# file_path = 'C:/Users/phily/liveability-index/data/uncertainty analysis/ua_minmax_450.csv'
# write_csv(ua_output_mm, file_path)
```
...
# file_path = 'C:/Users/phily/liveability-index/data/uncertainty analysis/ua_ss_450.csv'
# write_csv(ua_output_ss, file_path)
```
...
# file_path = 'C:/Users/phily/liveability-index/data/uncertainty analysis/ua_robust_450.csv'
# write_csv(ua_output_rs, file_path)
```
...
file_path = 'C:/Users/phily/liveability-index/data/uncertainty analysis/ua_data_final_450.csv'
...
## 6.4. Distribution of SD in mean neighbourhood ranking across different factor values

### 6.4.1. Assign neighbourhoods to London Boroughs
...
```{r SD in mean rank of neighbourhoods}
...
dev_normalisation <- results %>% 
  ...
  group_by(nb_clus) %>%
  summarise(rank_dev = sd(mean_rank) / 399, test = 'normalisation')

dev_diversity <- results %>% 
  group_by(nb_clus, diversity_weight) %>% 
  summarise(mean_rank = mean(li_rank)) %>%
  group_by(nb_clus) %>%
  summarise(rank_dev = sd(mean_rank) / 399, test = 'diversity_weight')

dev_proximity <- results %>% 
  group_by(nb_clus, proximity_weight) %>% 
  summarise(mean_rank = mean(li_rank)) %>%
  group_by(nb_clus) %>%
  summarise(rank_dev = sd(mean_rank) / 399, test = 'proximity_weight')

dev_pop_density <- results %>%
  group_by(nb_clus, pop_density_weight) %>%
  summarise(mean_rank = mean(li_rank)) %>%
  group_by(nb_clus) %>%
  summarise(rank_dev = sd(mean_rank) / 399, test = 'pop_density_weight')


# bind all tables together - 1596 rows
dev_all <- bind_rows(dev_normalisation, dev_diversity, dev_proximity, dev_pop_density)

# check
head(dev_all)

# check we have 399 records for each test
dev_all %>% group_by(test) %>% dplyr::summarise(count = n())

# save data
write_csv(dev_all, 'C:/Users/phily/liveability-index/data/uncertainty analysis/rank_dev_all_450.csv')
```

## 6.5. Plots 
### 6.5.1. Neighbourhood deviation in mean rank across factor values Plot
```{r}
# Custom names for x-axis labels
custom_names <- c("Normalisation Factor", "Population Density Factor", "Proximity Factor", "Diversity Factor")
...
myplot2
ggsave('C:/Users/phily/liveability-index/output/uncertainty analysis/sd_mean_rank_450.png', plot = myplot2, width = 10, height = 8, dpi = 300)
```
...
### 6.5.2. LAD Rankings for all simulations
```{r}
...
myplot1<- ggplot(lad_rank_for_288_tests, aes(x = LAD, y = LAD_rank)) +
  ...
  labs(x = "Local Area District (LAD)", y = "Rank") +
...
ggsave('C:/Users/phily/liveability-index/output/uncertainty analysis/ua_rank_simulation_450.png', plot = myplot1, width = 12, height = 12, dpi = 300)
```

### 6.5.3. LAD Rankings by normalisation factor
```{r plot ranking by normalisation factor}
mean_scores_by_test <- results %>%
  group_by(LAD, test, normalisation) %>%
  summarise(mean_li_score = mean(li_score))

# Step 2: Calculate the rank of each borough under each of the 288 different test conditions. Boroughs with the highest li_score will have the rank of 32, while the one with the lowest will be assigned of a rank of 1.
...
myplot2 <- ggplot(lad_rank_for_288_tests_norm, aes(x = LAD, y = LAD_rank)) +
  ...
  labs(x = "Local Area District (LAD)", y = "Rank", color = "Normalisation") +
  ggtitle("Liveability Rank of Boroughs under Different Simulations") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 11.5, family = 'sans'),
        axis.text.y = element_text(size = 11.5, family = 'sans'),
...
ggsave('C:/Users/phily/liveability-index/output/uncertainty analysis/ua_rank_simulation_norm_450.png', plot = myplot2, width = 12, height = 12, dpi = 300)
```

### 6.5.4. LAD Rankings by diversity
...
myplot3 <- ggplot(lad_rank_for_288_tests_norm, aes(x = LAD, y = LAD_rank)) +
  ...
  labs(x = "Local Area District (LAD)", y = "Rank", color = "Diversity Indicators") +
  ggtitle("Liveability Rank of Boroughs under Different Simulations") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 11.5, family = 'sans'),
        axis.text.y = element_text(size = 11.5, family = 'sans'),
        axis.title = element_text(size = 14, family = 'sans'),
        plot.title = element_text(face = "bold", size = 20, hjust = 0.5, family = 'sans', margin = margin(b = 20))) +
  scale_color_manual(values = my_colors, labels = c('1' = 'Original',
                                                    '2' = 'Diversity of Commerce',
                                                    '3' = 'Diversity of Education',
                                                    '4' = 'Diversity of Entertainment',
                                                    '5' = 'Diversity of Living',
                                                    '6' = 'Diversity of Healthcare'))
...
ggsave('C:/Users/phily/liveability-index/output/uncertainty analysis/ua_rank_simulation_div_450.png', plot = myplot3, width = 12, height = 12, dpi = 300)
```


### 6.5.5. Average shift in London Borough Rankings - $\overline{R_{LAD}}$

```{r}
# Step 1: By grouping neighbourhoods by boroughs and test, we can calculate the aggregated average li_score for each borough. Due to the small size of city of London, there are no neighbourhoods found in the area. Moreover, City of London is an administrative boundary. Thus, the liveability of City of London cannot be compared as there is no comparable neighbourhoods when neighbourhoods are constructed with 600m isodistance, City of London has been excluded.

mean_scores_by_test <- results %>%
  filter(LAD != 'City of London') %>%
  group_by(LAD, test) %>%
  summarise(mean_li_score = mean(li_score))
...
baseline <- lad_rank_for_288_tests %>%
  ...
  rename(LAD_rank_b = LAD_rank) %>% 
  select(LAD, LAD_rank_b)

# step 4: merge 288 scenarios with baseline data
...
# step 5: average shift in rank
avg_sft_LAD <- merged_data %>%
  group_by(test) %>%
  summarise(sum_rank_change = sum(li_rank_change),
            Rs = sum_rank_change/32)

# step 7: mean and sd of average shift in rank
sd <- sd(avg_sft_LAD$Rs) # 0.334
mean <- mean(avg_sft_LAD$Rs) # 1.238

# step 8: save file
file_path = 'C:/Users/phily/liveability-index/data/uncertainty analysis/average_shift_lad_450.csv'
write.csv(avg_sft_LAD, file_path, row.names = F)
```
...
plot1 <- 
  ...
  tm_layout(legend.position = c("right", "bottom"), 
            #title= "Liveability Profiles of 399 Neighbourhoods in London", 
...
# plot1
tmap_save(plot1, 'C:/Users/phily/liveability-index/output/results/overall_LI.pdf', height = 6, width = 8, dpi = 300)
```
...
tm_shape(lci_1) +
  ...
  tm_scale_bar(text.size = 0.9, position = c("left", "bottom")) +
  tm_layout(legend.position = c("right", "bottom"), 
            title= "Diversity Domain Profiles of 399 Neighbourhoods in London", 
            title.position = c('center', 'top'),
            title.size = 30,
            frame = FALSE,  # Remove the box surrounding the map
            inner.margins = 0.1)
...
tm1 <- 
  ...
  tm_legend(show = F) +
  tm_layout(frame = F) +
  tm_credits('Population Density Domain', position =c(0.11,0.92), size=1.5)  
...
tm2 <- 
  ...
  tm_layout(frame = F) +
  tm_credits("Diversity Domain", position = c(0.24,0.92), size=1.5)
...
tm3 <- 
  ...
  tm_legend(show = F) +
  tm_layout(frame = F) +
  tm_credits("Proximity Domain", position = c(0.24,0.92), size=1.5)
...
legend <- tm_shape(lci_1) +
  tm_polygons('div_decile',
              ...
              title = 'Decile',
              style = 'cat') +
  tm_scale_bar(position = c(0.315, 0.2), text.size = 0.9) +
...
tmap_save(t, 'C:/Users/phily/liveability-index/output/results/combined_domain.pdf', height = 9, width = 10, dpi = 300)
```



## 7.7. Decomposition of Indicators 
```{r}
# left join with LCI
lci_2 <- left_join(lci,
                   neighbourhood_lon,
...
# read in data
df <- read.csv('C:/Users/phily/liveability-index/data/domain data/raw_domain_indicator_nb_450.csv')
baseline <- read.csv('C:/Users/phily/liveability-index/data/uncertainty test/li_reference_rank_450.csv')
...
for (w in seq(1, nrow(weights))) {
  ...
  li_w1 <- li_w1 %>% mutate(li_rank = min_rank(li_score))
...
file_path = 'C:/Users/phily/liveability-index/data/sensitivity analysis/sa_dirichlet_diversity_results_1000.csv'
...
file_path = 'C:/Users/phily/liveability-index/data/sensitivity analysis/sa_dirichlet_proximity_results_1000.csv'
...
for (w in seq(1, nrow(weights))) {
  ...
  #li_fin <- select(all_fin, nb_clus, li_exp, li_score)
  
  # final index
  li_w1 <- all_fin %>% mutate(li_rank = min_rank(li_score))
...
file_path = 'C:/Users/phily/liveability-index/data/sensitivity analysis/sa_dirichlet_results_1000.csv'
...
```{r data import}
# data read
data <- read.csv('C:/Users/phily/liveability-index/data/sensitivity analysis/sa_dirichlet_results_1000.csv')
...
plot <- ggplot(data_summary, aes(x = alpha, y = mean_rank_change, color = w_focus)) +
  geom_line() +
...
# Save the plot
ggsave(filename = "C:/Users/phily/liveability-index/output/sensitivity analysis/sensitivity_analysis_plot.png", plot = plot, width = 10, height = 6, dpi = 300)
...
# data read
data_1 <- read.csv('C:/Users/phily/liveability-index/data/sensitivity analysis/sa_dirichlet_diversity_results_1000.csv')
...
# Create the line graph with markers
plot <- ggplot(data_summary_1, aes(x = alpha, y = mean_rank_change, color = w_focus)) +
  geom_line() +
  geom_point(data = markers, aes(x = alpha, y = mean_rank_change), shape = 19, size = 3) +  # Add markers
...
# Save the plot
ggsave(filename = "C:/Users/phily/liveability-index/output/sensitivity analysis/sensitivity_analysis_diversity_plot.png", plot = plot, width = 10, height = 6, dpi = 300)
```

### 8.3.3. Proximity Domain Indicators
```{r data import}
# data read
data_1 <- read.csv('C:/Users/phily/liveability-index/data/sensitivity analysis/sa_dirichlet_proximity_results_1000.csv')
...
# Convert w_focus to factor and rename
data_summary_1$w_focus <- factor(data_summary_1$w_focus, levels = c(1, 2, 3, 4, 5), labels = c("Proximity to Commerce", "Proximity to Education", "Proximity to Entertainment", 'Proximity to Living', 'Proximity to Healthcare'))
...
# Save the plot
ggsave(filename = "C:/Users/phily/liveability-index/output/sensitivity analysis/sensitivity_analysis_proximity_plot.png", plot = plot, width = 10, height = 6, dpi = 300)
```
...
ggsave(file_path, plot, dpi = 300, width = 6, height = 6)
```
...
## 10.3.  Difference in Active Travel Frequency between Neighbourhoods
...
### 10.3.1. Normality Test - Shapiro-wilk Test
```{r normality test}
...
```{r}
# Print the test results for Group 1 - p-value = 1.698e-08
print(shapiro_test_group1)
```

```{r}
# Print the test results for Group 2 - p-value = 0.3046
print(shapiro_test_group2)
```
The distribution of high liveability neighbourhood group was significantly different from normal distribution. However, that of low liveability neighbourhood group showed a normal distribution.
...
plot
file_path = 'C:/Users/phily/liveability-index/output/active travel/normalised_fs_by_nb_groups.png'
ggsave(file_path, plot, dpi = 300, height = 5, width = 7)
```
Given that high liveability neighbourhood does not follow a normal distribution while the other does, the appropriate statistical test to compare the difference in active travel patterns between the two groups would be a **non-parametric test**. **Non-parametric tests** do not assume normality and are suitable for comparing distributions when at least one group does not follow a normal distribution.

Therefore, the Mann-Whitney U Test (Wilcoxon Rank-Sum Test) would be the most straightforward and appropriate method to compare the difference in active travel patterns between the high and low liveability neighbourhoods, considering the non-normal distribution in one of the groups.
...
# Calculate the effect size (r) - 0.872
...
## 10.4. Correlation between Active Travel Frequency and LCI
```{r}
# correlation test
...
# Calculate Adjusted R-squared - 0.592
summary(lm_model)$adj.r.squared
...
file_path = 'C:/Users/phily/liveability-index/output/active travel/ff_li_correlation.png'
ggsave(file_path, plot, dpi = 300, width = 6, height = 6)
```